...
function claimsFromSources({ spotifyArtistId, wikidata, discogs, musicBrainz }) {
    ...
    for (const value of wikidata.musicBrainzArtistIds || []) claims.push({ identifierType: "musicBrainzArtistId", value, source: "wikidata", path: "P434", independentOrigin: "wikidata" });
    for (const value of wikidata.discogsArtistIds || []) claims.push({ identifierType: "discogsArtistId", value, source: "wikidata", path: "P1953", independentOrigin: "wikidata" });
...
function compareIdentifierClaims(claims) {
    ...
    const identifierComparison = Object.fromEntries(types.map(type => {
        const relevant = claims.filter(claim => claim.identifierType === type && claim.value != null);
...
function compareIdentityEvidence(input) {
    ...
    const compared = compareIdentifierClaims(claims);
    return {
        identifierComparison: compared.identifierComparison,
        conflicts: compared.conflicts,
        supportingMetadata: {
            names: [
                { source: "wikidata", value: input.wikidata?.name || null },
                { source: "discogs", value: input.discogs?.name || null },
                { source: "musicbrainz", value: input.musicBrainz?.name || null }
            ],
            musicBrainz: { artistType: input.musicBrainz?.artistType || null, country: input.musicBrainz?.country || null, area: input.musicBrainz?.area || null, beginArea: input.musicBrainz?.beginArea || null, lifeSpan: input.musicBrainz?.lifeSpan || null, aliases: input.musicBrainz?.aliases || [] },
            discogs: { realName: input.discogs?.realName || null, aliases: input.discogs?.aliases || [], profile: input.discogs?.profile || null }
        },
...
module.exports = { auditSameNameMusicBrainzEntities, claimsFromSources, compareIdentifierClaims, compareIdentityEvidence };
