...
async function lookupMusicBrainzArtistsByExactSpotifyUrl(spotifyArtistUrl, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        try {
            ...
            throw new MusicBrainzApiError("MusicBrainz exact Spotify-URL request failed before receiving a response.", null, error);
        }
        if (response.status === 503 && attempt < maxRetries) {
            const retrySeconds = Number(response.headers?.get?.("retry-after")) || (attempt + 1);
            await waitImpl(Math.max(1000, retrySeconds * 1000));
        ...
        if (response.status === 404) return { spotifyArtistUrl: resource, exactUrlFound: false, candidates: [], raw: null };
        if (!response.ok) throw new MusicBrainzApiError(`MusicBrainz exact Spotify-URL request failed with status ${response.status}.`, response.status);
        let raw;
        try { raw = await response.json(); } catch (error) {
            throw new MusicBrainzApiError("MusicBrainz exact Spotify-URL lookup returned invalid JSON.", response.status, error);
        }
        const candidates = normalizeExactUrlArtists(raw);
        return { spotifyArtistUrl: resource, exactUrlFound: true, candidates, raw };
    }
}
...
module.exports = { MusicBrainzApiError, getMusicBrainzArtist, getMusicBrainzArtistResolution, lookupMusicBrainzArtistsByExactSpotifyUrl, normalizeArtist, normalizeExactUrlArtists, searchMusicBrainzArtistsByExactName, validateMbid, validateSpotifyArtistUrl };
