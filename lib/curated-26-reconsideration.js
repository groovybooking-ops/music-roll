...
const { generalizedProviderMultiplicity } = require("./identity-conflict-resolution");
...
function buildReconsiderationReport(input) {
    ...
    const exactRecordsComplete = mbids.length > 0 && mbids.every(id => (input.musicBrainzRecords || {})[id]?.artist || (input.musicBrainzRecords || {})[id]?.musicBrainzArtistId)
    ...
    else if (!spotifyConfirmedByMusicBrainz || !exactRecordsComplete || sameNameAudit.sameNameAmbiguity === "not_evaluated") outcome = "insufficient_corroboration";
